...
ByteArray.prototype.reverse = function() {
	...
	if (this._cow) {
		...
		buffer.set(limit,this._buffer.get(limit));
	} else {
		buffer = this._buffer;
	}
	
	for (var i = 0, j = this._length - 1; i < limit; i++, j--) {
		var tmp = this._buffer.get(i);
		buffer.set(i,this._buffer.get(j));
		buffer.set(j,tmp);
	}
...
ByteArray.prototype.sort = function(comparator) {
	...
	Array.prototype.sort.call(arr, comparator || defaultComparator);
	this._buffer = new ByteArray(arr)._buffer;
 ...
 */
ByteArray.prototype.forEach2 = function(callback, thisObject) {
	for (var i = 0, length = this._length; i < length; i++) {
		callback.call(thisObject, this._buffer.get(i), i, this);
	}
};
...
ByteArray.prototype.forEach = function(callback, thisObject) {
	...
	for (var i = 0, length = this._length; i < length; i++) {
		callback.call(thisObject, arr[i], i, this);
	}
};
 ...
 */
ByteArray.prototype.filterX = function(callback, thisObject) {
	var result = [];
	for (var i = 0, length = this._length; i < length; i++) {
	...
	bytes_loop: while (currentOffset < length) {
		...
		delimiters_loop: for (var i = 0; i < delimiters.length; i++) {
			...
			for (var j = 0; j < d.length; j++) {
				...
				if (currentOffset + j > length || buffer.get(currentOffset + j) !== d._buffer.get(j)) {
...
Binary.prototype.toArray = function(charset) {
	...
	if (charset) {
		...
		return Array.prototype.slice.call(this._buffer.toArray(), 0, this._length);
	}
};
...
Binary.prototype.indexOf = function(byteValue, start, stop) {
	...
	return Array.prototype.indexOf.call(buffer.toArray(), byteValue, start);
};
...
Binary.prototype.lastIndexOf = function(byteValue, start, stop) {
	...
	var result = Array.prototype.lastIndexOf.call(buffer.toArray(), byteValue);
...
ByteString.prototype.byteAt = ByteString.prototype.charAt = function(offset) {
	if (offset < 0 || offset >= this._length) {
		...
		return new ByteString([this._buffer.get(offset)]);
	}
};
...
Binary.prototype.get = ByteString.prototype.charCodeAt = function(offset) {
	if (0 <= offset && offset < this._length) {
		return this._buffer.get(offset);
		// -- replacedd -- return this._buffer.get[offset];
	}
};
